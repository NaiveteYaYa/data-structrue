...
def findMinArrowShots(points: List[List[int]]) -> int:
    """
    解法：
    初始公共相交区域范围，init_range
        开始循环，设置第一个气球为初始边界
        每次循环，检测边界范围，与新的气球直径是否有交集
            如果有交集，边界继续缩小
            若没有交集，则弓箭数量+1，init_range = 新气球范围。
            直到循环结束
    :param points:
    :return:
    """
    ...
    init_range = []
    for point in sort_p:
        if init_range and init_range[1] >= point[0]:
            init_range[0] = point[0]
            if point[1] <= init_range[1]:
                init_range[1] = point[1]
        else:
            init_range = point
            res += 1
